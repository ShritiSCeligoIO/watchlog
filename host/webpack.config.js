import path from 'node:path';
import { fileURLToPath } from 'node:url';
import dotenv from 'dotenv';
import Dotenv from 'dotenv-webpack';
import HtmlWebpackPlugin from 'html-webpack-plugin';
import MiniCssExtractPlugin from 'mini-css-extract-plugin';
import webpack from 'webpack';
import { BundleAnalyzerPlugin } from 'webpack-bundle-analyzer';
import CssMinimizerPlugin from 'css-minimizer-webpack-plugin';
import {
  DEFAULT_REMOTE_URL,
  REMOTE_ENTRY_FILENAME,
  REMOTE_NAME,
  sharedDependencies,
} from '../webpack/federation.js';

const { ModuleFederationPlugin } = webpack.container;
const root = path.dirname(fileURLToPath(import.meta.url));
const source = path.join(root, 'src');

dotenv.config({ path: path.join(root, '.env') });
dotenv.config({ path: path.join(root, '.env.defaults') });
const remoteUrl = process.env.REMOTE_WATCHLOG_URL || DEFAULT_REMOTE_URL;

export default (env = {}, argv = {}) => {
  const production = argv.mode === 'production';

  return {
    mode: production ? 'production' : 'development',
    entry: path.join(source, 'index.tsx'),
    devtool: production ? 'source-map' : 'eval-cheap-module-source-map',
    output: {
      path: path.join(root, 'build'),
      publicPath: 'auto',
      clean: true,
      filename: production
        ? 'assets/[name].[contenthash:8].js'
        : 'assets/[name].js',
      chunkFilename: production
        ? 'assets/[name].[contenthash:8].chunk.js'
        : 'assets/[name].chunk.js',
    },
    resolve: {
      extensions: ['.tsx', '.ts', '.jsx', '.js', '.json'],
      extensionAlias: { '.js': ['.ts', '.tsx', '.js'] },
    },
    module: {
      rules: [
        {
          test: /\.[jt]sx?$/,
          include: source,
          use: {
            loader: 'babel-loader',
            options: {
              presets: [
                ['@babel/preset-env', { modules: false, bugfixes: true }],
                ['@babel/preset-react', { runtime: 'automatic' }],
                '@babel/preset-typescript',
              ],
            },
          },
        },
        {
          test: /\.css$/,
          use: [
            production ? MiniCssExtractPlugin.loader : 'style-loader',
            'css-loader',
          ],
        },
      ],
    },
    plugins: [
      new HtmlWebpackPlugin({
        template: path.join(source, 'index.html'),
        publicPath: '/',
      }),
      new Dotenv({
        path: path.join(root, '.env'),
        defaults: path.join(root, '.env.defaults'),
        systemvars: true,
        silent: true,
      }),
      new ModuleFederationPlugin({
        name: 'host',
        remotes: {
          [REMOTE_NAME]: `${REMOTE_NAME}@${remoteUrl}/${REMOTE_ENTRY_FILENAME}`,
        },
        shared: sharedDependencies(),
      }),
      ...(production
        ? [
            new MiniCssExtractPlugin({
              filename: 'assets/[name].[contenthash:8].css',
            }),
          ]
        : []),
      ...(env.analyze
        ? [
            new BundleAnalyzerPlugin({
              analyzerMode: 'static',
              reportFilename: '../../reports/host.html',
              statsFilename: '../../reports/host-stats.json',
              generateStatsFile: true,
              openAnalyzer: false,
            }),
          ]
        : []),
    ],
    optimization: {
      runtimeChunk: false,
      usedExports: true,
      sideEffects: true,
      minimizer: ['...', new CssMinimizerPlugin()],
      splitChunks: {
        chunks: 'async',
        cacheGroups: {
          vendors: {
            test: /[\\/]node_modules[\\/]/,
            name: 'vendors',
            reuseExistingChunk: true,
          },
        },
      },
    },
    devServer: {
      port: 3000,
      historyApiFallback: true,
      hot: true,
    },
    performance: { hints: false },
    stats: 'errors-warnings',
  };
};

export { remoteUrl };
